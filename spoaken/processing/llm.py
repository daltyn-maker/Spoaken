...
_ollama_ok        = None    # None = untested; True/False = last result
_ollama_ok_ts     = 0.0     # epoch-monotonic seconds of last check
_OLLAMA_TTL_S     = 30.0    # re-probe at most every 30 s
...
def is_ollama_running() -> bool:
    ...
    Result is cached for _OLLAMA_TTL_S seconds so repeated calls within a
    session do not spam the network, while still re-checking after Ollama
    starts or stops rather than caching False permanently for the session.
    """
    global _ollama_ok, _ollama_ok_ts
    import time as _time
    now = _time.monotonic()
    if _ollama_ok is not None and (now - _ollama_ok_ts) < _OLLAMA_TTL_S:
        return _ollama_ok
    try:
...
def translate(
    ...
    # deep_translator sends text to Google's API.
    # FIX: spoaken_config has no is_online() function — that import caused an
    # ImportError that silently swallowed all translation fallback attempts.
    # Removed. deep_translator raises its own network errors when offline.
    try:
...
def background_process_and_export(
    new_text    : str,
    mode        : str,             # "summarize" | "translate"
    ...
    """
    Process *new_text* with the LLM (summarize or translate) and append the
    result to the appropriate log file in Logs/.

    Designed to be called from a background thread.

    Parameters
    ----------
    new_text    : Incremental transcript text to process.
    mode        : "summarize" or "translate".
    target_lang : Target language (only used when mode=="translate").
    model       : Ollama model override (None = auto-pick).
    ratio       : Summary ratio for extractive fallback.
    log_dir     : Override log directory path.  Defaults to paths.LOG_DIR.
    ...
    try:
        if mode == "summarize":
            result   = summarize_llm(new_text, model=model, ratio=ratio)
            log_path = os.path.join(_log_dir, "llm_summary.txt")
            prefix   = "[LLM Summary]"
        elif mode == "translate":
            result   = translate(new_text, target_lang=target_lang, model=model)
            log_path = os.path.join(_log_dir, "llm_translation.txt")
            prefix   = f"[LLM Translation → {target_lang}]"
        else:
            print(f"[LLM Export]: unknown mode '{mode}'", file=sys.stderr)
            return None
        ...
        print(f"[LLM Export]: {mode} failed — {exc}", file=sys.stderr)
