...
───────────────────────
Extractive text summarizer for Spoaken.
...
def _split_sentences(text: str) -> list[str]:
    ...
    cleaned = re.sub(
        r"\b(Mr|Mrs|Ms|Dr|Prof|Sr|Jr|vs|etc|i\.e|e\.g)\.", r"\1<DOT>", text
    )
...
def _term_frequencies(words: list[str]) -> dict[str, float]:
    """Normalized term frequency, ignoring stopwords."""
...
def summarize_extractive(
    ...
    """
    Extractive summarization via sentence TF scoring with positional weighting.
    ...
    top_indices = sorted(
        sorted(range(len(scored)), key=lambda i: scored[i][2], reverse=True)[
...
def summarize(text: str, ratio: float = 0.33, max_sentences: int = 12) -> str:
    ...
    Falls back cleanly to extractive summarization if no neural model is
