...
class VAD:
    ...
    def __init__(
        ...
        if WEBRTCVAD_AVAILABLE:
            try:
                self._vad = _webrtcvad_module.Vad(aggressiveness)
    ...
    def set_aggressiveness(self, level: int):
        ...
        if WEBRTCVAD_AVAILABLE:
            try: self._vad = _webrtcvad_module.Vad(level)
    ...
    def _frame_is_speech(self, frame: bytes) -> bool:
        if self._vad:
            try: return self._vad.is_speech(frame, self._sample_rate)
            except Exception: pass
