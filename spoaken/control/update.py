...
class DownloadProgressWindow(ctk.CTkToplevel):
    ...
    def _build_ui(self, title: str):
        ...
        _logo_img = None
        try:
            from spoaken.system.paths import ART_DIR as _art
...
class SpoakenUpdater(ctk.CTkToplevel):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            if _png.exists():
                try:
                    img = _Image.open(_png).resize((64, 64))
                    self._icon = _ImageTk.PhotoImage(img)
                    self.after(200, lambda: self.wm_iconphoto(True, self._icon))
                except Exception:
                    pass
        except Exception:
            pass  # PIL / paths unavailable — skip icon
    ...
    def _on_install_t5(self):
        ...
        Uses a DownloadProgressWindow for progress — does NOT touch the
        package-update busy flag so the main Update button stays usable.
        """
        # No _busy check here: T5 download is independent of pip operations.
    ...
    def _download_t5_worker(self, model_id: str, label: str,
        ...
        try:
            ...
            if dpw:
                dpw.mark_done(False)
        # NOTE: do NOT call _set_busy here — this worker runs inside a
        # DownloadProgressWindow, not the package-update button flow.
    ...
    def _check_versions_worker(self):
        ...
        self._log("Fetching latest versions from PyPI (concurrent) …")
    ...
    def _on_install_vosk(self):
        ...
        package-update busy flag so the main Update button stays usable.
        """
        # No _busy check: Vosk model download is independent of pip operations.
    ...
    def _on_install_whisper(self):
        ...
        package-update busy flag so the main Update button stays usable.
        """
        # No _busy check: Whisper model download is independent of pip operations.
    ...
    def _download_vosk_worker(self, model_name: str, url: str, size_mb: int = 0, dpw: DownloadProgressWindow = None):
        ...
        try:
            try:
                from spoaken.system.paths import VOSK_DIR
            ...
            # Temp cleanup only — do NOT call _set_busy here.
            # Vosk download uses its own DownloadProgressWindow.
            pass
    ...
    def _download_whisper_worker(self, model_name: str, size_mb: int = 0, dpw: DownloadProgressWindow = None):
        ...
        try:
            ...
            pass
            # Do NOT call _set_busy here — Whisper download
            # uses its own DownloadProgressWindow.
