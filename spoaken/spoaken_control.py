...
# ── Log rotation config (5 MB per file, 3 backups kept) ───────────────────────
_LOG_MAX_BYTES  = 5 * 1024 * 1024   # 5 MB
_LOG_BACKUP_COUNT = 3


def _make_rotating_logger(name: str, filepath: str) -> logging.Logger:
    """Return a logger that writes to a size-rotating file handler."""
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    def __init__(self):
        ...
        try:
            from spoaken_config import T5_MODEL as _T5_MODEL
    ...
    def _ensure_logs(self):
        ...
        # Create rotating loggers (will also create the files if absent)
        self._vosk_logger    = _make_rotating_logger("spoaken.vosk",    VOSK_LOG)
        self._whisper_logger = _make_rotating_logger("spoaken.whisper", WHISPER_LOG)
        self._final_logger   = _make_rotating_logger("spoaken.final",   FINAL_LOG)
    ...
    # ── T5 text-to-text model controls ────────────────────────────────────────

    def set_t5_enabled(self, enabled: bool):
        """Enable or disable the T5 grammar/paraphrase engine."""
        self._t5_enabled = enabled

    def set_t5_mode(self, mode, model: str = None):
        """Set T5 processing mode ('correct' | None) and optionally the model."""
        self._t5_mode  = mode
        if model:
            self._t5_model = model

    def set_t5_model(self, model: str):
        """Change the active T5 model without altering mode."""
        self._t5_model = model
        # Persist selection to config so it survives restarts
        try:
    ...
    def run_t5_correction(self, text: str = None) -> str:
        """
        ...
        try:
            while self.model.is_running:
                ...
                if rec.AcceptWaveform(data):
                    ...
                    self._vosk_logger.info(f"[Final]: {text}\n")
                    ...
                    if partial and partial != last_partial:
                        ...
                        self._vosk_logger.info(f"[Partial]: {partial}\n")
