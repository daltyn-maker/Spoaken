...
def summarize_extractive(
    ...
    """
    Extractive summarisation via sentence TF scoring.
    ...
    scored = [(i, s, _score_sentence(s, tf)) for i, s in enumerate(sentences)]

    # Number of sentences to keep
    k = max(1, min(max_sentences, round(n * ratio)))

    # Select top-k by score, restore original order
    top_indices = sorted(
        sorted(range(len(scored)), key=lambda i: scored[i][2], reverse=True)[:k]
    )
